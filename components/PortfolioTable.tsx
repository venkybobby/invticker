...
export function PortfolioTable() {
  const { holdings, addHolding, removeHolding, updateHolding } = useForgeStore();
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400">Allocation: <span className={totalAllocation > 103 || totalAllocation < 97 ? 'text-red-400' : 'text-emerald-400'}>{totalAllocation.toFixed(1)}%</span></p>
        </div>
        <button onClick={addHolding} className="flex items-center gap-x-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-2xl text-sm font-semibold">
          ...
          <tbody className="divide-y divide-slate-800">
            {holdings.map((holding) => (
              <tr key={holding.id} className="group">
                ...
                <td className="px-6 py-4">
                  <input value={holding.notes} onChange={(e) => updateHolding(holding.id, 'notes', e.target.value)} className="table-input w-full" placeholder="Thesis..." />
                </td>
                <td className="px-4 py-4">
                  <button onClick={() => removeHolding(holding.id)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-500 p-2">
      ...
      <div className="mt-4 text-right text-sm text-slate-400">
        Total: <span className="font-mono text-white">${totalValue.toLocaleString()}</span>
      </div>
    </div>
  );
}
